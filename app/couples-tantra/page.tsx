"use client";

import Footer from "@/components/Footer";
import LeadForm from "@/components/LeadForm";
import UserReviews from "@/components/UserReviews";
import FAQSection from "@/components/FAQSection";

export default function CouplesTantraPage() {
  return (
    <div className="bg-cream min-h-screen">
      {/* Landing Page Hero */}
      <section className="relative pt-12 pb-16 md:py-24 overflow-hidden bg-transparent">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            {/* Left Info Column */}
            <div className="lg:col-span-7 space-y-6">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-accent-red/10 border border-accent-red/20 rounded-full text-xs font-bold text-accent-red uppercase tracking-wider">
                💕 Intimacy & Connection
              </div>
              <h1 className="font-title text-4xl sm:text-5xl lg:text-6xl font-black text-deep-brown leading-tight">
                Deepen Intimacy with <span className="text-accent-red italic">Couples Tantra</span>
              </h1>
              <p className="font-body text-lg text-deep-brown/80 leading-relaxed max-w-xl">
                Are you looking to rebuild trust, harmonize your partner energies, or deepen your emotional and spiritual intimacy? Experience supportive, virtual Couples Tantra and relationship healing guided with absolute safety and respect.
              </p>
              
              {/* Benefits Bullets */}
              <div className="space-y-3.5 pt-4">
                {[
                  "Harmonize partner energies and resolve subtle communication blocks",
                  "Learn sacred, non-physical breathing and meditation rituals together",
                  "Grounded, practical methods without uncomfortable or trendy pop setups",
                  "Fully private, 100% confidential virtual sessions from home",
                ].map((b, i) => (
                  <div key={i} className="flex items-start gap-3">
                    <span className="text-accent-red font-bold text-lg mt-0.5">✔</span>
                    <span className="font-body text-base font-semibold text-deep-brown/85">{b}</span>
                  </div>
                ))}
              </div>

              {/* Trust Indicators */}
              <div className="pt-6 border-t border-deep-brown/10 flex flex-wrap gap-6 text-xs font-bold text-deep-brown/65 uppercase tracking-widest">
                <span>🔒 Secure & Confidential</span>
                <span>💻 Joint Online Sessions</span>
                <span>⭐ 4.9/5 satisfaction</span>
              </div>
            </div>

            {/* Right Form Column */}
            <div className="lg:col-span-5 relative z-25">
              <LeadForm defaultGoal="Relationship Healing" buttonText="Book Our Free Couples Call" />
            </div>
          </div>
        </div>
      </section>

      {/* Detail Section */}
      <section className="py-20 bg-white border-y border-deep-brown/5">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h2 className="font-title text-3xl sm:text-4xl font-bold text-deep-brown text-center mb-12">
            What Couples Experience
          </h2>
          
          <div className="space-y-8">
            {[
              {
                t: "Energetic Alignment",
                d: "Learn to recognize and align the polarities of energy within your relationship, shifting from conflict to conscious connection.",
              },
              {
                t: "Synchronized Breathing (Pranayama)",
                d: "Guided breathing exercises designed to slow down the nervous system and bring both partners into a state of shared presence.",
              },
              {
                t: "Intimacy Rituals",
                d: "Simple, powerful practices that cultivate sacred geometry and eye-gazing to re-establish deep trust and safety.",
              },
            ].map((item, idx) => (
              <div key={idx} className="p-6 bg-cream/35 border border-deep-brown/5 rounded-2xl">
                <h3 className="font-title text-xl font-bold text-deep-brown mb-2">{item.t}</h3>
                <p className="font-body text-sm sm:text-base text-deep-brown/70 leading-relaxed">{item.d}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* FAQ & Reviews */}
      <UserReviews />
      <FAQSection />
      
      <Footer />
    </div>
  );
}
