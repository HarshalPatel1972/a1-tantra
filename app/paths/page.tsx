    ...
    <div>
      ...
      <div className="pt-32 pb-24 bg-cream min-h-screen">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {tantraPaths.map((path) => (
              <Link
                key={path.id}
                href={`/paths/${path.id}`}
                className="group block p-8 border-2 border-mystique-taupe/20 hover:border-accent-red bg-white hover:bg-white/70 rounded-lg transition"
              >
                <div className="text-5xl mb-4">{path.icon}</div>
                <h2 className="serif-heading text-2xl font-bold text-mystique-taupe mb-3 group-hover:text-accent-red transition">
                  {path.name}
                </h2>
          ...
          <div className="mt-16 p-8 bg-mystique-taupe text-cream rounded-lg text-center">
            ...
            <Link
              href="/start-journey"
              className="inline-block px-6 py-3 bg-accent-red hover:bg-soft-gold hover:text-mystique-taupe transition font-semibold rounded-sm text-cream"
            >
              TAKE THE QUIZ
            </Link>
          </div>
